...
namespace Engine
{
	bool PixelEngine::Construct(vi2d size, FullscreenMode fullscreen, RefreshRate refresh, int subSamples) noexcept
	{
		...
		m_aspectRatio = AspectRatio((float)m_vScreenSize.x / (float)m_vScreenSize.y);
	...
	void PixelEngine::selectSubFrame(const vu2d start, const vu2d size) const noexcept
	{
		selectSubFrame(start.x, start.y, size.x, size.y);
	}

	void PixelEngine::selectSubFrame(const uint32_t x, const uint32_t y, const uint32_t width, const uint32_t height) const noexcept
	{
		glViewport(x, y, width, height);
	}

	void PixelEngine::resetSubFrame() const noexcept
	{
		glViewport(0, 0, GetScreenSize().x, GetScreenSize().y);
	}

	void PixelEngine::DrawSprite(const Sprite2D &sprite, const vf2d &pos, const vf2d &size, const Pixel tint) const noexcept
	{
		Decal(sprite, pos, size).render(m_sSpriteShader, tint);
	}

	void PixelEngine::DrawSpriteWorld(const Sprite2D &sprite, const TransformedView &tv, const vf2d &pos, const vf2d &size, const Pixel tint) const noexcept
	{
		Decal(sprite, pos, size).renderWorld(m_sSpriteShader, tv, tint);
	}

	void PixelEngine::DrawSprite(const Sprite2D &sprite, const vf2d &tl, const vf2d &tr, const vf2d &bl, const vf2d &br, const Pixel tint) const noexcept
	{
		Decal(sprite, tl, tr, bl, br).render(m_sSpriteShader, tint);
	}

	void PixelEngine::DrawDecal(const Decal &decal, const Pixel tint) const noexcept
	{
		decal.render(m_sSpriteShader, tint);
	}

	void PixelEngine::DrawDecalWorld(const Decal &decal, const TransformedView &tv, const Pixel tint) const noexcept
	{
	...
	{
		getAsciiTable(res).bind();
		decal.renderWorld(getSpriteShader(), tv, col);
	}

	Sprite2D PixelEngine::getAsciiTable(const Resolution res) const noexcept
	{
		return m_pStringTables[(uint32_t)res];
	}

	AspectRatio PixelEngine::getAspectRatio() const noexcept
	{
		return m_aspectRatio;
	}

	SpriteShader PixelEngine::getSpriteShader() const noexcept
	{
		return m_sSpriteShader;
	}
